...
$stmt = $pdo->prepare("
    ...
    WHERE d.proker_id = ? AND d.jenis = 'surat'
...
$upload_message = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['surat_file'])) {
    ...
    if (!in_array($file_ext, $allowed_extensions)) {
        ...
        $new_filename = 'surat_' . $proker_id . '_' . time() . '.' . $file_ext;
        ...
        if (move_uploaded_file($tmp_name, $file_path)) {
            ...
            if ($stmt->execute([$proker_id, $filename, 'uploads/surat/' . $new_filename, $_SESSION['user_id']])) {
                $upload_message = '<div class="alert alert-success">Surat berhasil diupload!</div>';
                // Refresh data
                $stmt = $pdo->prepare("
                    SELECT d.*, u.nama as uploaded_by_name 
                    FROM dokumentasi d 
...
<head>
    ...
    <title>Upload Surat - <?= sanitize($proker['nama_proker']) ?></title>
        ...
        <div class="dashboard">
            <div class="stats-bar">
                ...
                <div style="text-align: right;">
                    <strong><?= count($surats) ?></strong><br>
                    <small>Surat tersimpan</small>
                </div>
            </div>
            ...
            <div class="upload-zone" onclick="document.getElementById('suratFile').click();">
                <div class="upload-icon">📤</div>
                <h3>Klik atau Drag & Drop untuk Upload Surat</h3>
                ...
                <form id="uploadForm" method="POST" enctype="multipart/form-data" style="display: inline;">
                    <input type="file" id="suratFile" name="surat_file" class="file-input" 
                           accept=".pdf,.doc,.docx" onchange="handleFileSelect(this)" />
                    <button type="button" class="btn btn-primary" onclick="document.getElementById('suratFile').click();">
                        Pilih File
                    </button>
                </form>
            </div>
            ...
            <?php if (count($surats) > 0): ?>
                <div class="proposals-grid">
                    <?php foreach ($surats as $surat): ?>
                        <div class="proposal-card" onclick="previewFile('<?= $surat['file_path'] ?>', '<?= sanitize($surat['nama_file']) ?>')">
                            <div class="proposal-icon">
                                <?= getFileIcon($surat['nama_file']) ?>
                            </div>
                            <div class="proposal-name">
                                <?= sanitize($surat['nama_file']) ?>
                            </div>
                            <div class="proposal-meta">
                                Diupload oleh: <?= sanitize($surat['uploaded_by_name']) ?><br>
                                <?= date('d/m/Y H:i', strtotime($surat['uploaded_at'])) ?>
                            </div>
                            <div class="proposal-actions" onclick="event.stopPropagation();">
                                <a href="download.php?id=<?= $surat['id'] ?>" 
                                   class="btn btn-success btn-small">Download</a>
                                <a href="delete_dokumentasi.php?id=<?= $surat['id'] ?>&proker_id=<?= $proker_id ?>&return_to=surat" 
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>Upload surat pertama Anda dengan mengklik area upload di atas</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
...
">
    ...
    <script>
        ...
        uploadZone.addEventListener('dragover', e => {
            e.preventDefault();
            uploadZone.style.backgroundColor = '#e0e7ff';
        });
        ...
        uploadZone.addEventListener('drop', e => {
            ...
            if (files.length > 0) {
                document.getElementById('suratFile').files = files;
